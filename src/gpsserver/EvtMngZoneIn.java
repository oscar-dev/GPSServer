 ...
 * @author omedi
 */
public class EvtMngZoneIn extends EvtMngGeoEvent {
    
    @Override
    public void checkEvent(Message message) throws SQLException, AddressException, MessagingException {
        int zone_id = 0;
        String sql = "SELECT zone_vertices, zone_id, zone_name FROM gs_user_zones WHERE user_id=" + this._user_id + " AND zone_id in(" + this._zones + ")";
        ...
        while(rs.next()) {
            ...
            //MainServer.logger.info("Val: " + Integer.toString(en_zona) + " Evento: " + this._type);
            
            if( en_zona == 1 ) {
                if( event_status == -1 ) {
                    this.set_status(zone_id);
